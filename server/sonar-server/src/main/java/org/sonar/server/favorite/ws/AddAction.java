...
public class AddAction implements FavoritesWsAction {
  ...
  private Consumer<AddRequest> addFavorite() {
    return request -> {
      try (DbSession dbSession = dbClient.openSession(false)) {
        ComponentDto componentDto = componentFinder.getByKey(dbSession, request.getComponent());
        userSession
          .checkLoggedIn()
  ...
  private static Function<Request, AddRequest> toWsRequest() {
    return request -> new AddRequest(request.mandatoryParam(PARAM_COMPONENT));
  }
}
